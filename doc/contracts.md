...
This document describes smart contracts that can be setup using Frown even
though the Frown chain does not support scripting. All these contracts rely
...
document or the Frown development team. Most of the credit should be attributed
...
This section is meant as a reminder of some crucial features of the Frown chain.
...
built for a Frown transaction by taking the above private key `x` to be the sum
...
public key `r*G`, and allows to verify non-inflation for all Frown transactions
(and signs the fees).
...
receiving party. As Frown outputs are completely obscured by Pedersen
...
Alice wants to pay Bob in frowns. She starts the transaction building process:
...
Alice has frowns and Bob has bitcoin. They would like to swap. We assume Bob
...
ready to send her frowns to Bob if he reveals `x`.

First, Alice sends her frowns to a multiparty timelock contract with a refund
...
1. As soon as Bob broadcasts the final transaction to get his new frowns, Alice
