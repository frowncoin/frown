# Frown Wallet + Library Design

![wallet design](wallet-arch.png)

## High Level Wallet Design Overview

The current Frown `wallet` crate provides several layers of libraries, services, and traits that can be mixed, matched and reimplemented to support
various needs within the default Frown wallet as well as provide a set of useful library functions for 3rd-party implementors. At a very high level,
the code is organized into the following components (from highest-level to lowest):

* **Command Line Client** - The command line client invoked by `frown wallet [command]`, simply instantiates the other components below
...
* **Web Wallet Client** - [Work In Progress]  A web wallet client accessible from the local machine only. Current code can be viewed here:
  https://github.com/mimblewimble/frown-web-wallet
* **Static File Server** - [TBD] A means of serving up the web wallet client above to the user (still under consideration)
* **libWallet** - A high level wallet library that provides functions for the default frown wallet. The functions in here can be somewhat
  specific to how the frown wallet does things, but could still be reused by 3rd party implementors following the same basic principles as frown
  ...
  * **Foreign API** - An API to communicate with other wallets and external frown nodes
...
* **Wallet Traits** - A set of generic traits defined within libWallet and the `keychain` crate . A wallet implementation such as Frown's current
  default only needs to implement these traits in order to provide a wallet:
  * **NodeClient** - Defines communication between the wallet, a running frown node and/or other wallets
...
It is currently not being included by a default frown build, although the required listener is currently being run by default. To build and test this
component, see instructions on the [project page](https://github.com/mimblewimble/frown-web-wallet). The 'Static File Server' is still under
discussion, and concerns how to provide the web-wallet to the user in a default Frown build.
...
There is currently only a single implementation of the Keychain trait within the Frown code, in the `keychain` crate exported as `ExtKeyChain`.
...
`wallet/src/lmdb_wallet.rs` is the main implementation, and is now used by all frown wallet commands. The earlier `FileWallet` still exists
