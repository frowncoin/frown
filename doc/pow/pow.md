# Frown's Proof-of-Work

This document is meant to outline, at a level suitable for someone without prior knowledge,
the algorithms and processes currently involved in Frown's Proof-of-Work system. We'll start
with a general overview of cycles in a graph and the Cuckoo Cycle algorithm which forms the
basis of Frown's proof-of-work. We'll then move on to Frown-specific details, which will outline
the other systems that combine with Cuckoo Cycle to form the entirety of mining in Frown.

Please note that Frown is currently under active development, and any and all of this is subject to
(and will) change before a general release.

## Graphs and Cuckoo Cycle

Frown's basic Proof-of-Work algorithm is called Cuckoo Cycle, which is specifically designed
...
technical aspects of Frown's proof-of-work.
...
Now, (hopefully) armed with a basic understanding of what the Cuckoo Cycle algorithm is intended to do, as well as the parameters that affect how difficult it is to find a solution, we move on to the other portions of Frown's POW system.

## Mining in Frown

The Cuckoo Cycle outlined above forms the basis of Frown's mining process, however Frown uses Cuckoo Cycle in tandem with several other systems to create a Proof-of-Work.
...
Contrary to some existing concerns about Cuckoo Cycle's poolability, the POW implementation in Frown as described above
is perfectly suited to a mining pool. While it may be difficult to prove efforts to solve a single graph in isolation,
the combination of factors within Frown's proof-of-work combine to enforce a notion called 'progress-freeness', which
...
Following from this, Frown's progress-freeness is due to the fact that a solution to a Cuckoo with Frown's default parameters
