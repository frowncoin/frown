...
//! Frown server implementation, glues the different parts of the system (mostly
...
use crate::frown::{dandelion_monitor, seed, sync};
...
/// Frown server holding internal structures.
pub struct Server {
	/// server config
	...
	/// Maintain a lock_file so we do not run multiple Frown nodes from same dir.
	lock_file: Arc<File>,
}
...
impl Server {
	...
	// instance of frown server from the same dir.
	// This uses fs2 and should be safe cross-platform unless somebody abuses the file itself.
	fn one_frown_at_a_time(config: &ServerConfig) -> Result<Arc<File>, Error> {
		let path = Path::new(&config.db_root);
		fs::create_dir_all(path.clone())?;
		let path = path.join("frown.lock");
		...
		lock_file.try_lock_exclusive().map_err(|e| {
			...
			writeln!(
				&mut stderr,
				"Failed to lock {:?} (frown server already running?)",
				path
			)
	...
	pub fn new(config: ServerConfig) -> Result<Server, Error> {
		...
		let lock_file = Server::one_frown_at_a_time(&config)?;
