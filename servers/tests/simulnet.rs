...
fn simulate_full_sync() {
	...
	let test_name_dir = "frown-sync";
	framework::clean_all_output(test_name_dir);

	let s1 = servers::Server::new(framework::config(1000, "frown-sync", 1000)).unwrap();
	...
	let s2 = servers::Server::new(framework::config(1001, "frown-sync", 1000)).unwrap();
...
fn simulate_fast_sync() {
	...
	let test_name_dir = "frown-fast";
	framework::clean_all_output(test_name_dir);

	// start s1 and mine enough blocks to get beyond the fast sync horizon
	let s1 = servers::Server::new(framework::config(2000, "frown-fast", 2000)).unwrap();
	let stop = Arc::new(Mutex::new(StopState::new()));
	s1.start_test_miner(None, stop.clone());
	...
	let mut conf = config(2001, "frown-fast", 2000);
...
fn simulate_long_fork() {
	...
	let test_name_dir = "frown-long-fork";
	framework::clean_all_output(test_name_dir);
...
fn long_fork_test_preparation() -> Vec<servers::Server> {
	...
	let mut conf = framework::config(2100, "frown-long-fork", 2100);
	conf.archive_mode = Some(false);
	...
	for i in 1..6 {
		let mut conf = config(2100 + i, "frown-long-fork", 2100);
		conf.archive_mode = Some(false);
		conf.api_secret_path = None;
...
/// Intended to replicate https://github.com/mimblewimble/frown/issues/1325
#[ignore]
#[test]
