...
use crate::consensus::FROWN_BASE;
...
pub fn amount_from_hr_string(amount: &str) -> Result<u64, Error> {
	...
	let (frowns, nfrowns) = match amount.find('.') {
		None => (parse_frowns(amount)?, 0),
		Some(pos) => {
			let (gs, tail) = amount.split_at(pos);
			(parse_frowns(gs)?, parse_nfrowns(&tail[1..])?)
		}
	};
	Ok(frowns * FROWN_BASE + nfrowns)
}

fn parse_frowns(amount: &str) -> Result<u64, Error> {
	if amount == "" {
		Ok(0)
	} else {
		amount
...
lazy_static! {
	static ref WIDTH: usize = (FROWN_BASE as f64).log(10.0) as usize + 1;
}

fn parse_nfrowns(amount: &str) -> Result<u64, Error> {
...
pub fn amount_to_hr_string(amount: u64, truncate: bool) -> String {
	let amount = (amount as f64 / FROWN_BASE as f64) as f64;
