...
impl UI {
	...
	pub fn new(controller_tx: mpsc::Sender<ControllerMessage>) -> UI {
		...
		title_string.append(StyledString::styled(
			format!("Frown Version {}", built_info::PKG_VERSION),
		...
		//set theme
		let mut theme = frown_ui.cursive.current_theme().clone();
		modify_theme(&mut theme);
		frown_ui.cursive.set_theme(theme);
		frown_ui.cursive.add_fullscreen_layer(main_layer);

		// Configure a callback (shutdown, for the first test)
		let controller_tx_clone = frown_ui.controller_tx.clone();
		frown_ui.cursive.add_global_callback('q', move |_| {
			controller_tx_clone
				.send(ControllerMessage::Shutdown)
				.unwrap();
		});
		frown_ui.cursive.set_fps(4);
		frown_ui
	}
