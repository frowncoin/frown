...
//! Main for building the binary of a Frown peer-to-peer node.
...
pub fn info_strings() -> (String, String) {
	(
		format!(
			"This is Frown version {}{}, built for {} by {}.",
		...
		("wallet", Some(wallet_args)) => {
			...
			if !cmd::seed_exists(w.members.as_ref().unwrap().wallet.clone()) {
				if "init" == wallet_args.subcommand().0 || "recover" == wallet_args.subcommand().0 {
					...
					println!("Wallet seed file doesn't exist. Run `frown wallet init` first");
					exit(1);
				}
			}
