...
/// Frown client commands processing
...
pub fn client_command(client_args: &ArgMatches<'_>, global_config: GlobalConfig) -> i32 {
	...
	match client_args.subcommand() {
		...
		_ => panic!("Unknown client command, use 'frown help client' for details"),
	}
	0
}
...
pub fn show_status(config: &ServerConfig, api_secret: Option<String>) {
	println!();
	let title = format!("Frown Server Status");
	...
	match get_status_from_node(config, api_secret) {
		...
		Err(_) => writeln!(
			e,
			"WARNING: Client failed to get data. Is your `frown server` offline or broken?"
		)
		.unwrap(),
	};
	e.reset().unwrap();
