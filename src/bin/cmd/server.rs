...
/// Frown server commands processing
...
fn start_server_tui(config: servers::ServerConfig) {
	...
	if config.run_tui.unwrap_or(false) {
		warn!("Starting FROWN in UI mode...");
...
/// stopping the Frown blockchain server. Processes all the command line
/// arguments to build a proper configuration and runs Frown with that
/// configuration.
pub fn server_command(
	...
	if let Some(a) = server_args {
		match a.subcommand() {
			...
			("", _) => {
				println!("Subcommand required, use 'frown help server' for details");
			}
			(cmd, _) => {
				println!(":: {:?}", server_args);
				panic!(
					"Unknown server command '{}', use 'frown help server' for details",
					cmd
				);
			}
		}
