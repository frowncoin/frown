...
mod wallet_tests {
	...
	use frown_wallet::test_framework::{self, LocalWalletClient, WalletProxy};

	use clap::{App, ArgMatches};
	use frown_util::Mutex;
	use std::sync::Arc;
	use std::thread;
	use std::time::Duration;
	use std::{env, fs};

	use frown_config::GlobalWalletConfig;
	use frown_core::global;
	use frown_core::global::ChainTypes;
	use frown_keychain::ExtKeychain;
	use frown_wallet::{LMDBBackend, WalletBackend, WalletConfig, WalletInst, WalletSeed};
	...
	pub fn config_command_wallet(
		...
		config_file_name.push("frown-wallet.toml");
		if config_file_name.exists() {
			return Err(frown_wallet::ErrorKind::ArgumentError(
				"frown-wallet.toml already exists in the target directory. Please remove it first"
					.to_owned(),
			))?;
		}
	...
	fn instantiate_wallet(
		...
		passphrase: &str,
		account: &str,
	) -> Result<Arc<Mutex<WalletInst<LocalWalletClient, ExtKeychain>>>, frown_wallet::Error> {
	...
	fn execute_command(
		...
		arg_vec: Vec<&str>,
	) -> Result<String, frown_wallet::Error> {
	...
	/// command line tests
	fn command_line_test_impl(test_dir: &str) -> Result<(), frown_wallet::Error> {
		setup(test_dir);
		...
		let yml = load_yaml!("../frown.yml");
		let app = App::from_yaml(yml);

		// wallet init
		let arg_vec = vec!["frown", "wallet", "-p", "password", "init", "-h"];
		...
		let arg_vec = vec![
			"frown", "wallet", "-p", "password", "account", "-c", "mining",
		];
		...
		let arg_vec = vec!["frown", "wallet", "-p", "password", "account"];
		execute_command(&app, test_dir, "wallet2", &client2, arg_vec)?;
		...
		let wallet1 = instantiate_wallet(config1.clone(), client1.clone(), "password", "default")?;
		frown_wallet::controller::owner_single_use(wallet1.clone(), |api| {
		...
		let arg_vec = vec!["frown", "wallet", "-p", "password", "-a", "mining", "info"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let wallet2 = instantiate_wallet(config2.clone(), client2.clone(), "password", "default")?;
		frown_wallet::controller::owner_single_use(wallet2.clone(), |api| {
		...
		let arg_vec = vec!["frown", "wallet", "-p", "password", "check"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
		...
		let arg_vec = vec![
			"frown", "wallet", "-p", "password", "-a", "mining", "cancel", "-i", "26",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec!["frown", "wallet", "-p", "password", "-a", "mining", "txs"];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// message output (mostly spit out for a visual in test logs)
		let arg_vec = vec![
			"frown", "wallet", "-p", "password", "-a", "mining", "txs", "-i", "10",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;

		// txs and outputs (mostly spit out for a visual in test logs)
		let arg_vec = vec![
			"frown", "wallet", "-p", "password", "-a", "mining", "outputs",
		];
		execute_command(&app, test_dir, "wallet1", &client1, arg_vec)?;
