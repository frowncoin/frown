...
fn mine_empty_chain() {
	...
	mine_some_on_top(".frown", pow::mine_genesis_block().unwrap(), &keychain);
}
...
fn mine_genesis_reward_chain() {
...

	{
		...
		let tmp_chain = setup(".frown.tmp", pow::mine_genesis_block().unwrap());
	...
	mine_some_on_top(".frown.genesis", genesis, &keychain);
}
...
fn mine_forks() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	let chain = setup(".frown2", pow::mine_genesis_block().unwrap());
...
fn mine_losing_fork() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	let kc = ExtKeychain::from_random_seed(false).unwrap();
	let chain = setup(".frown3", pow::mine_genesis_block().unwrap());
...
fn longer_fork() {
	...
	let chain = setup(".frown4", genesis.clone());
...
fn spend_in_fork_and_compact() {
	global::set_mining_mode(ChainTypes::AutomatedTesting);
	util::init_test_logger();
	let chain = setup(".frown6", pow::mine_genesis_block().unwrap());
	let prev = chain.head_header().unwrap();
	let kc = ExtKeychain::from_random_seed(false).unwrap();
