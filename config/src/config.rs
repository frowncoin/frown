...
pub const SERVER_CONFIG_FILE_NAME: &'static str = "frown-server.toml";
/// And a wallet configuration file name
pub const WALLET_CONFIG_FILE_NAME: &'static str = "frown-wallet.toml";
const SERVER_LOG_FILE_NAME: &'static str = "frown-server.log";
const WALLET_LOG_FILE_NAME: &'static str = "frown-wallet.log";
const FROWN_HOME: &'static str = ".frown";
const FROWN_CHAIN_DIR: &'static str = "chain_data";
/// Wallet data directory
pub const FROWN_WALLET_DIR: &'static str = "wallet_data";
/// API secret file name
pub const API_SECRET_FILE_NAME: &'static str = ".api_secret";

fn get_frown_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
	// Check if frown dir exists
	let mut frown_path = match dirs::home_dir() {
		Some(p) => p,
		None => PathBuf::new(),
	};
	frown_path.push(FROWN_HOME);
	frown_path.push(chain_type.shortname());
	// Create if the default path doesn't exist
	if !frown_path.exists() {
		fs::create_dir_all(frown_path.clone())?;
	}
	Ok(frown_path)
}
...
fn check_api_secret_file(chain_type: &global::ChainTypes) -> Result<(), ConfigError> {
	let frown_path = get_frown_path(chain_type)?;
	let mut api_secret_path = frown_path.clone();
...
pub fn initial_setup_server(chain_type: &global::ChainTypes) -> Result<GlobalConfig, ConfigError> {
	...
	// Use config file if current directory if it exists, .frown home otherwise
	if let Some(p) = check_config_current_dir(SERVER_CONFIG_FILE_NAME) {
		...
		let mut config_path = frown_path.clone();
		...
		if !config_path.exists() {
			...
			default_config.update_paths(&frown_path);
...
impl GlobalConfig {
	...
	/// Update paths
	pub fn update_paths(&mut self, frown_home: &PathBuf) {
		// need to update server chain path
		let mut chain_path = frown_home.clone();
		chain_path.push(FROWN_CHAIN_DIR);
		self.members.as_mut().unwrap().server.db_root = chain_path.to_str().unwrap().to_owned();
		let mut secret_path = frown_home.clone();
		...
		let mut log_path = frown_home.clone();
...
impl GlobalWalletConfig {
	...
	pub fn update_paths(&mut self, wallet_home: &PathBuf) {
		...
		wallet_path.push(FROWN_WALLET_DIR);
