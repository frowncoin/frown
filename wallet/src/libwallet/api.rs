	...
	K: Keychain,
{
	...
	/// # Example
	/// ```
	/// # extern crate frown_wallet as wallet;
	/// # extern crate frown_keychain as keychain;
	/// # extern crate frown_util as util;
	///
	/// use std::sync::Arc;
	/// use util::Mutex;
	///
	...
	/// * `amount` - The amount to send, in nanofrowns. (`1 G = 1_000_000_000nG`)
	/// * `minimum_confirmations` - The minimum number of confirmations an output
	...
	///		Some("Have some Frowns. Love, Yeastplume".to_owned()),
	///	);
	///
	...
	/// Builds the complete transaction and sends it to a frown node for
	/// propagation.
	...
	pub fn cancel_tx(
		...
		if !self.update_outputs(&mut w, false) {
			return Err(ErrorKind::TransactionCancellationError(
				"Can't contact running Frown node. Not Cancelling.",
			))?;
		}
