...
impl HTTPNodeClient {
	/// Create a new client that will communicate with the given frown node
...
impl NodeClient for HTTPNodeClient {
	...
	/// Posts a transaction to a frown node
...
pub fn create_coinbase(dest: &str, block_fees: &BlockFees) -> Result<CbData, Error> {
	...
	match single_create_coinbase(&url, &block_fees) {
		Err(e) => {
			error!(
				"Failed to get coinbase from {}. Run frown wallet listen?",
				url
			);
